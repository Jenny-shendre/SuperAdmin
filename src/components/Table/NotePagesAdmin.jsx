  ...
  >
    {["All", "Client", "Manager", "Super Admin"].map((tab) => (
      <button
        ...
        className={`  ${
          activeTab === tab ? "bg-[#3D2314] text-white" : " text-[#3D2314] "
        } ${tab === "All" ? "rounded-l-[24px] " : ""} ${
          tab === "Super Admin" ? "rounded-r-[24px] " : ""
        }`}
...
function NotePagesAdmin() {
  ...
  const handleSubmit = (e) => {
    ...
    setProject('');
    setBriefing('');
    setButtonText('Note Successfully Added ✓');
    setError('');
          ...
          >
            <div className="add-team-members w-[688px] h-auto p-6 rounded-lg bg-white shadow-lg flex flex-col items-center">
              ...
              >
                ...
                {isProjectDropdownOpen && (
                  <div className="absolute z-10 mt-2 w-full p-2 bg-white text-[#3D2314] border border-gray-300 rounded-md shadow-lg max-h-52 overflow-y-auto">
                    {data2.map((projects) => (
                      <div
                        key={projects.name}
                        className="p-2 cursor-pointer hover:bg-gray-200"
